...
const Pos = () => {
  ...
  const onProductList = () => {
    axios
      .get("https://6682bba24102471fa4c8096e.mockapi.io/Products")
      .then((response) => {
  ...
  const summary = useMemo(() => {
    let total =
      carts.reduce((nilaiUpdate, obj) => nilaiUpdate + obj.subtotal, 0) -
      pembayaran.discount;
    if (total < 0) {
      total = 0;
    }
      ...
      <Container className={"mt-5"}>
        <Row>
          <Col
            md={8}
            className={
              "d-flex flex-wrap justify-content-evenly row-gap-4 column-gap-1"
            }
          >
            {products.map((obj) => (
              <Card
              ...
              >
                ...
                <Card.Body>
                  <Card.Title>{obj.produk}</Card.Title>
                  <Card.Text className={"text-truncate mb-5"}>
                    {obj.deskripsi}
                  </Card.Text>
                  <ButtonGroup className="d-flex mt-3 gap-5 justify-content-center">
                ...
                <tbody>
                  {carts.map((obj) => (
                    <tr key={obj.id}>
                      ...
                      {product.id && product.id === obj.id ? (
                        ...
                        <td onClick={() => onSelectProduct(obj)}>
                          {obj.quantity}
                        </td>
                      )}
                  ...
                  <tr>
                    ...
                    <td className={"fw-bold fs-5"}>
                      {summary === 0 ? "FREE" : formatCurrency(summary)}
                    </td>
                  </tr>
                  <tr>
